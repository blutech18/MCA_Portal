...
@push('head')
  ...
  <style>
    ...
    .declined-enrollees-page {
      padding: 0;
      margin: 0;
    }

    /* Remove any unwanted text or elements */
    .declined-enrollees-page::before {
      content: none;
    }

    /* Hide any unwanted text that might appear */
    .declined-enrollees-page > *:not(.filter-controls):not(.strand-box):not(.modal) {
              ...
              <tr data-enrollee-id="{{ $e->id }}" data-enrollee-name="{{ $e->display_name }}" data-enrollee-type="new">
                ...
                <td>
                  @if($e->payment_status === 'verified')
                    Verified
                  @elseif($e->payment_status === 'pending' || $e->payment_status === 'Pending Verification')
                    Pending
                  @else
                    Not Set
                  @endif
